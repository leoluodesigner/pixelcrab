...
export function generateStaticParams() {
  return getContentByType('news', 'everything-is-crab').map((item) => ({
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; id: string }> }): Promise<Metadata> {
  ...
  const item = game ? getContentItem(game.id, 'news', id) : undefined;
  if (!game || !item) return {};

  return {
    title: item.title,
    description: item.summary,
    alternates: { canonical: `/games/${game.slug}/news/${item.slug}` },
    openGraph: {
      title: item.title,
      description: item.summary,
      url: absoluteUrl(`/games/${game.slug}/news/${item.slug}`),
